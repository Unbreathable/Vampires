...
import de.nightempire.servercore.particle.ParticleBuilder;
...
public class IngameState extends GameState {
        ...
        Vampires.getInstance().getTaskManager().inject(runnable = new Runnable() {
            ...
            public void run() {
                ...
                if(tickCount++ >= 20) {
                    ...
                    if(prison.size() >= Vampires.getInstance().getGameManager().getTeamManager().getTeam("Menschen").getPlayers().size()) {
                        handleWin(Vampires.getInstance().getGameManager().getTeamManager().getTeam("Vampire"));
                    }
        ...
        if(team instanceof VampireTeam) {
            ...
            for(BeetrootData data : beetroots) {
                if(data.location.distance(event.getPlayer().getLocation()) <= 3) {
                    ...
                    event.getPlayer().addPotionEffect(new PotionEffect(PotionEffectType.SLOW, 100, 0));
    ...
    public void onDamage(EntityDamageEvent event) {
        if(event.getEntity() instanceof Player player) {
            ...
            if(team.getName().equals("Menschen")) {
                event.setDamage(0);
            }
    ...
    public void handleDeath(Player player) {
        ...
        } else Bukkit.broadcastMessage(Vampires.PREFIX + "§c§l" + player.getKiller().getName() + " §7ist an einem niedrigen §cBlutlevel §7gestorben!");
        ...
        if(team.getPlayers().size() == 0) {
            handleWin(Vampires.getInstance().getGameManager().getTeamManager().getTeam("Menschen"));
        }
    }
    ...
    public static class BeetrootData {
        ...
        public BeetrootData(Location location) {
            ...
            item = (Item) location.getWorld().spawnEntity(location.clone().add(0, 1, 0), EntityType.DROPPED_ITEM);
            item.setItemStack(new ItemStackBuilder(Material.BEETROOT).buildStack());
            item.setVelocity(new Vector(0, 0, 0));
            item.setPickupDelay(1000000);
            item.setUnlimitedLifetime(true);
            item.setCustomNameVisible(true);
            item.setCustomName("§c§lBlut Knoblauch");
